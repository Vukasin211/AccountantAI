...
def plot_results(dates, actuals, preds, correct_mask, overall_accuracy, accuracy_threshold=PCT_THRESHOLD):
    N = len(actuals)
    ...
    plt.title(f"Rolling predictions (window={WINDOW_SIZE}) — overall correct (±{int(accuracy_threshold*100)}%): {overall_accuracy:.2f}%")
    plt.xlabel("Date")
...
def simulate(
    ...
    buf = plot_results(pred_dates, actuals_clean, preds, correctness, overall_accuracy, accuracy)
    buf.seek(0)

    return StreamingResponse(buf, media_type="image/png")

@app.post("/simulateHistorical")
def simulateHistorical(
    max_days: int = Query(None, description="Limit how many days to simulate (default = all)"),
    accuracy: float = Query(0.30, description="Accuracy threshold as decimal (default 0.30 = 30%)")
):
    """
    Rolling walk-forward simulation using historical data for validation
    """
    ...
    for i in range(start_idx, end_idx):
        ...
        window_dates = dates_daily[i-WINDOW_SIZE:i]
        last_real_amount = float(amounts_daily[i-1])
        model_label, model_obj = choose_model_by_value(last_real_amount)
        X = seq_to_model_input(window_scaled, window_dates)
        pred_scaled = float(model_obj.predict(X, verbose=0)[0][0])
        pred_amount = inverse_amount(pred_scaled)
        ...
        correct_mask.append(abs(pred_amount-actual_amount)/actual_amount <= accuracy if actual_amount!=0 else True)
    ...
    print(f"DEBUG - First 5 dates: {[d.strftime('%Y-%m-%d') for d in pred_dates[:5]]}")
    print(f"DEBUG - First 5 actuals: {actuals_list[:5]}")
    print(f"DEBUG - First 5 predictions: {preds_list[:5]}")

    buf = plot_results(pred_dates, y_true, y_pred, correct_mask, pct_within, accuracy)
    buf.seek(0)
    return StreamingResponse(buf, media_type="image/png")
...
def root():
    return {
        ...
        "endpoints": {
            ...
            "simulateHistorical": "POST /simulateHistorical (optional query params max_days, accuracy) - returns PNG plot", 
